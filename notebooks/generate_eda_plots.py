...
from gridsight.models.lstm.base.lstm_q import LSTMQuantile
...
MAX_NIGHT_SAMPLES = 800
NIGHT_ELEVATION_THRESHOLD = -5.0
NIGHT_GENERATION_THRESHOLD = 50.0
MAX_SCATTER_SAMPLES = 1500
EMPIRICAL_EFFICIENCY_SLOPE = 0.12
CALIBRATION_FACTOR = 0.990
LOOK_BACK_STEPS = 96
...
# Pandera Schema for Gold dataset validation
gold_schema = pa.DataFrameSchema({
    "timestamp_utc": pa.Column(nullable=False),
    "target_mw": pa.Column(checks=pa.Check.greater_than_or_equal_to(0.0), nullable=True),
    "target_cf": pa.Column(checks=pa.Check.in_range(0.0, 1.5), nullable=True),
    "ssrd_uk": pa.Column(checks=pa.Check.greater_than_or_equal_to(0.0), nullable=True),
    "tcc_uk": pa.Column(checks=pa.Check.in_range(0.0, 1.0), nullable=True),
    "lcc_uk": pa.Column(checks=pa.Check.in_range(0.0, 1.0), nullable=True),
    "t2m_uk": pa.Column(nullable=True),
    "ws10_uk": pa.Column(checks=pa.Check.greater_than_or_equal_to(0.0), nullable=True),
    "solar_elevation_deg": pa.Column(checks=pa.Check.in_range(-90.0, 90.0), nullable=True),
    "clearsky_cos": pa.Column(checks=pa.Check.in_range(0.0, 1.0), nullable=True),
    "capacity_mwp": pa.Column(checks=pa.Check.greater_than_or_equal_to(0.0), nullable=True),
}, strict=False)


def generate_bronze_eda(output_dir, df_gold):
...
def generate_gold_eda(output_dir, df_gold):
    ...
    corr_df = df_gold[corr_cols].dropna()
    ...
    df_gold["season"] = df_gold["timestamp_utc"].dt.month.map(get_season)
    
    diurnal = df_gold.groupby(["season", "half_hour"])["target_cf"].mean().reset_index()
    ...
    df_sample = df_gold.sample(min(MAX_SCATTER_SAMPLES, len(df_gold)), random_state=42)
    ...
    ys = np.minimum(xs * EMPIRICAL_EFFICIENCY_SLOPE, 100)
...
def generate_forecast_case_study(output_dir, df_gold):
    ...
    # Import and construct LSTMQuantile wrapper directly to remove redundancy and prevent drift
    model_wrapper = LSTMQuantile(cfg, len(features))
    model_wrapper.build()
    model_wrapper.model_.load_state_dict(torch.load(lstm_weights_path, map_location="cpu"))
    model = model_wrapper.model_
    model.eval()
        
    df = df_gold.sort_values('timestamp_utc').reset_index(drop=True)
    ...
    # Find indices for our target window, including steps look_back before it
    ...
    with torch.no_grad():
        for i in range(look_back, len(df_window)):
            x_seq = torch.tensor(X_window[i - look_back + 1 : i + 1], dtype=torch.float32).unsqueeze(0).to(device)
            pred = model(x_seq).cpu().numpy()[0]
            all_preds.append(pred)
            
    raw = np.vstack(all_preds)
    
    # Calibrate predictions using optimal factor
    q10_raw = raw[:, 0]
    q50_raw = raw[:, 1]
    q90_raw = raw[:, 2]
    
    q10_raw = q50_raw - (q50_raw - q10_raw) * CALIBRATION_FACTOR
    q90_raw = q50_raw + (q90_raw - q50_raw) * CALIBRATION_FACTOR
...
def main():
    ...
    try:
        # Load the Gold dataset ONCE to avoid duplicate loading overhead
        print("📂 Loading Gold features dataset...")
        files_gold = sorted(glob.glob("data/gold/gold_features_h48/**/*.parquet", recursive=True))
        if not files_gold:
            files_gold = sorted(glob.glob("data/gold/gold_features/**/*.parquet", recursive=True))
            
        if not files_gold:
            raise FileNotFoundError("No Gold features parquet files found in data/gold/")
        ...
        # Validate schema using Pandera to enforce strict quality contracts
        print("🔍 Validating Gold dataset schema using Pandera...")
        gold_schema.validate(df_gold)
        print("✅ Gold dataset schema validation passed!")
        
        # Execute diagnostic tasks passing the loaded/validated dataset
        generate_bronze_eda(output_dir, df_gold)
        generate_gold_eda(output_dir, df_gold)
        generate_feature_importance(output_dir)
        generate_forecast_case_study(output_dir, df_gold)
