...
def _build_cfg(a) -> ModelConfig:
    ...
    cfg = dataclasses.replace(
        ...
        seq_len=a.seq_len, tcn_epochs=a.epochs, lstm_epochs=a.lstm_epochs,
        val_start=a.val_start, test_start=a.test_start,
        daylight_only=not a.no_daylight,
    )
    if a.fast:  # quick smoke config
        cfg = dataclasses.replace(
            cfg, tcn_epochs=2, lstm_epochs=2, n_folds=3, seq_len=48,
...
def main() -> None:
    p = argparse.ArgumentParser(description="Train TCN-Q + LGBM-Q + LSTM-Q -> Linear-Q stack")
    ...
    p.add_argument("--epochs", type=int, default=30, help="TCN epochs")
    p.add_argument("--lstm-epochs", type=int, default=30, help="LSTM epochs")
