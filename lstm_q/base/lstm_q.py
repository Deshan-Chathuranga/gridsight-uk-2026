"""LSTM-Q base learner: an LSTM Network with a quantile head.

Captures sequential context from past half-hourly features. One forward pass
    ...
    class LSTMModel(nn.Module):
        def __init__(self):
            ...
            self.head = nn.Sequential(
                ...
                nn.GELU(),
                nn.Linear(cfg.lstm_hidden // 2, n_quantiles),
            )

        def forward(self, x):
            out, _ = self.lstm(x)      # (B, L, H)
            last = out[:, -1, :]       # (B, H)
            return self.head(last)     # (B, Q)
...
class LSTMQuantile:
    ...
    def fit(self, seqs: np.ndarray, y: np.ndarray) -> "LSTMQuantile":
        import torch
        ...
        opt = torch.optim.AdamW(
            ...
            lr=self.cfg.lstm_lr,
            weight_decay=self.cfg.lstm_weight_decay
        )

        ds = TensorDataset(torch.from_numpy(seqs), torch.from_numpy(y))
        dl = DataLoader(ds, batch_size=self.cfg.lstm_batch, shuffle=True, drop_last=False)
        E = self.cfg.lstm_epochs
        for ep in range(E):
            self.model_.train()
            tot, nb = 0.0, 0
            for xb, yb in dl:
            ...
            if ep == 0 or (ep + 1) % max(1, E // 5) == 0 or ep == E - 1:
                logger.info(f"    LSTM[{dev}] epoch {ep+1}/{E} · pinball={tot/max(nb,1):.4f}")
        return self
