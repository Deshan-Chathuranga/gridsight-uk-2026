# `lstm_q/` — Probabilistic solar forecast: Standalone LSTM-Q Model
...
| `clearsky.py` | Clear-Sky GHI (Haurwitz) helper for features/calibration |
| `base/lstm_q.py` | PyTorch LSTM model wrapper, custom pinball loss, and device selector |
| `metrics.py` | pinball loss, coverage (PICP), crossing rate, and skill score vs NESO |
...
./venv/bin/python -m lstm_q.tune --trials 15 --epochs 40

# Render evaluation fan chart and dashboard plots:
./venv/bin/python -m lstm_q.evaluate --split test
```
