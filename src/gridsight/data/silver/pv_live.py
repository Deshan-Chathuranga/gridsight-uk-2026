...
def build_silver_pv_live() -> pd.DataFrame:
    ...
    df = raw[raw["gsp_id"] == 0].copy()  # national
    ...
    n_neg = clamp_to_nan(df, "generation_mw")  # range incl. negatives -> NaN
    if n_neg:
    ...
    df["capacity_mwp"] = df["capacity_mwp"].ffill().bfill()  # capacity is slow-moving
