"""Leakage-safe lag & rolling features on OBSERVED actuals.
"""
...
def add_lags_rolling(df: pd.DataFrame, horizon: int) -> pd.DataFrame:
    ...
    # OCF observed fleet index (also lagged by horizon)
    if ocf is not None:
        for L in lag_set(horizon):
            df[f"ocf_lag_{L}"] = ocf.shift(L).astype("float32")
        oh = ocf.shift(horizon)
        df["ocf_roll_mean_48"] = oh.rolling(48, min_periods=24).mean().astype("float32")

    # warmup flag: rows before enough history exists for the longest feature
