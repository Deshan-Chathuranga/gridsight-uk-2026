"""Deterministic time features: calendar + solar geometry.
"""
...
def add_calendar(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["half_hour"] = (ts.hour * 2 + (ts.minute // 30)).astype("int16")  # 0..47
...
def solar_position(index: pd.DatetimeIndex, lat: float, lon: float):
    ...
    time_offset = eqtime + 4.0 * lon            # lon east-positive, minutes
...
def add_solar(df: pd.DataFrame, lat: float = UK_LAT, lon: float = UK_LON) -> pd.DataFrame:
    ...
    df["clearsky_cos"] = csky.astype("float32")     # 0 at night, ~theoretical insol factor
