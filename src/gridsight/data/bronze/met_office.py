...
# Lambert azimuthal equal-area params (constant for this dataset's grid).
_MET_CRS_CF = {
...
# Fallback points if configs/uk_weather_points.csv is missing (region, lat, lon).
_MET_DEFAULT_POINTS = [
...
def _met_out_path(repo_file: str) -> Path:
    """bronze/met_office_nwp/year=YYYY/month=MM/nwp_YYYYMMDD-HHZ.parquet"""
    stem = Path(repo_file).stem.replace(".zarr", "")  # 2023-01-01-00
...
def _extract_points_from_zip(local_zip: str, repo_file: str,
    ...
    """Open one .zarr.zip and sample the 5 vars at the 7 points across all leads."""
    import re
    import numpy as np
    import zarr
    ...
    try:
        ...
        # nearest grid index per point (grid identical across groups -> compute once)
        ...
        for g in group_names:
            ...
            if col is None:
                continue  # variable we don't keep
            _grp, inner = _MET_VARMAP[col]
            vals = np.asarray(zg[g][inner].get_coordinate_selection((ys, xs)))
            valid_time = pd.Timestamp(valid_s, tz="UTC")  # group prefix IS valid time
...
def _extract_one(repo_file: str, points: list[tuple[str, float, float]],
    ...
    try:
        ...
        shutil.rmtree(tmp, ignore_errors=True)  # keep peak disk to a few temp files
...
def ingest_met_office_nwp(
    ...
    """Thin Bronze for Met Office NWP: point-extract 5 vars at 7 UK regions.

    For each selected init-time we stream the .zarr.zip to a temp file, sample the
    7 points, write a tiny parquet, then delete the temp file. Peak disk ~ a few
    x 143 MB; final Bronze ~ 12 KB/init. `hours` filters init-times/day.
    """
    try:
    ...
    # For year 2026+, also fetch live forecast from Open-Meteo
    if any(y >= 2026 for y in years):
        try:
            logger.info("Ingesting live met_office_nwp from Open-Meteo for year 2026+...")
            from .fetch_met_office_live import fetch_and_save_live_nwp
            fetch_and_save_live_nwp()
        except Exception as e:
            logger.error(f"Failed to fetch live NWP data: {e}")

    targets = _met_office_targets(api, years, months, hours)
    if not targets:
        if any(y >= 2026 for y in years):
            logger.info("No Hugging Face targets found for 2026+, but live Open-Meteo ingestion was completed.")
            return
