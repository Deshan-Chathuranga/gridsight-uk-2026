...
# GridSight-relevant default datasets (CKAN package ids).
NESO_DEFAULT_PACKAGES = ["embedded-wind-and-solar-forecasts"]
_CKAN_MIN_INTERVAL = 1.1  # seconds between requests (policy: <= 1 req/s)
...
def ingest_neso(
    ...
    """Mirror NESO CKAN datasets to Bronze parquet (one parquet per CSV resource).

    packages      CKAN package ids (default: GridSight-relevant set).
    years         keep only archive resources whose name contains one of these
                  years (resources with no year in the name are always kept).
    force_refresh re-download even if last_modified is unchanged.
    max_records   cap rows per resource (dev/testing).
    """
    try:
        import httpx
