...
def _build_torch(cfg, n_features: int, n_quantiles: int, use_weight_norm: bool = True):
    import torch
    ...
    class TemporalBlock(nn.Module):
        def __init__(self, ci, co, k, d, dropout):
            super().__init__()
            pad = (k - 1) * d
            conv1 = nn.Conv1d(ci, co, k, padding=pad, dilation=d)
            conv2 = nn.Conv1d(co, co, k, padding=pad, dilation=d)
            if use_weight_norm:
                conv1 = nn.utils.weight_norm(conv1)
                conv2 = nn.utils.weight_norm(conv2)
            self.net = nn.Sequential(
                conv1, Chomp1d(pad), nn.ReLU(), nn.Dropout(dropout),
                conv2, Chomp1d(pad), nn.ReLU(), nn.Dropout(dropout),
            )
...
class TCNQuantile:
    ...
    def build(self, use_weight_norm: bool = True):
        """Construct the (untrained) network — used when reloading saved weights."""
        import torch
        torch.set_num_threads(self.cfg.torch_threads)
        self.model_ = _build_torch(self.cfg, self.n_features, len(self.quantiles), use_weight_norm=use_weight_norm)
        return self
