# `src/gridsight/models/stacking/` — Probabilistic solar forecast: TCN-Q + LGBM-Q → Linear-Q stack
...
| `common/metrics.py` | Consolidated pinball, coverage (PICP), crossing rate, skill vs NESO helper under common models |
...
./venv/bin/python -m gridsight.models.stacking --fast          # quick smoke run
./venv/bin/python -m gridsight.models.stacking                 # full train + evaluate
./venv/bin/python -m gridsight.models.stacking --target target_mw --epochs 40
```
...
!python -m gridsight.models.stacking --epochs 40          # full GPU training
```
