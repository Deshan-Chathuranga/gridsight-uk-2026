...
Pipeline:
  ...
  6. Track parameters, metrics, and models with MLflow.
...
def run(cfg: ModelConfig) -> dict:
    ...
    mlflow.set_experiment("GridSight-Stacking-Training")
    with mlflow.start_run(run_name="Stacking-Q Pipeline Run"):
        # Log config params
        mlflow.log_params({
            "target": cfg.target,
            "seq_len": cfg.seq_len,
        ...
        mlflow.log_artifact(str(out_dir / "stack.joblib"))
        mlflow.log_artifact(str(out_dir / "tcn.pt"))
