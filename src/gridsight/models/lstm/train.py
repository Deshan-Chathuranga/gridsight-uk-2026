...
This pipeline:
  ...
  6. Tracks hyperparameter config, epoch metrics, final evaluations, and artifacts with MLflow.
"""
...
def _lstm_fit_with_early_stopping(cfg, seqs, seqpos_of_row, tr_rows, val_rows, y, n_features) -> LSTMQuantile:
    ...
    for ep in range(E):
        ...
        # log epoch metrics to MLflow if run is active
        if mlflow.active_run() is not None:
            mlflow.log_metric("train_pinball_loss", tr_avg, step=ep)
            mlflow.log_metric("val_pinball_loss", val_avg, step=ep)
...
def run_lstm_pipeline(cfg: ModelConfig) -> dict:
    ...
    mlflow.set_experiment("GridSight-LSTM-Training")
    with mlflow.start_run(run_name="LSTM-Q Pipeline Run"):
        # Log config params
        mlflow.log_params({
            "target": cfg.target,
            "seq_len": cfg.seq_len,
        ...
        mlflow.log_params({
            "calibration_w": best_w,
            "calibration_k": best_k,
            "val_coverage": best_cov,
        })
        ...
        for name, rows_split in [("val", val_rows), ("test", test_rows)]:
            ...
            # Log validation & test metrics to MLflow
            for metric_name, metric_val in rep.items():
        ...
        mlflow.log_artifact(str(out_dir / "lstm.joblib"))
        mlflow.log_artifact(str(out_dir / "lstm.pt"))
