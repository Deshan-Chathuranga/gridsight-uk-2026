"""Evaluation charts for the trained LSTM-Q model.

Reads the per-slot predictions saved by `train.py` (artifacts/lstm/pred_test.parquet)
and renders evaluation figures (PNG) you can view on Colab or download:

  * evaluation_fan.png        — q10–q90 band vs actual over a window (MW)
  * evaluation_dashboard.png  — calibration, q50 scatter, error-by-hour, metric card

Run:  python -m lstm_q.evaluate            
      python -m lstm_q.evaluate --split val
"""
...
from gridsight.models.common import metrics
...
def _load(artifacts_dir: str | Path, split: str):
    ...
    if not f.exists():
        raise FileNotFoundError(f"{f} not found — run `python -m lstm_q.train` first.")
...
def make_charts(artifacts_dir: str | Path = "artifacts/lstm", split: str = "test"):
    import matplotlib
    matplotlib.use("Agg")
    ...
    df = _load(out, split)
    rep = _metrics(df)
    print(f"=== LSTM-Q {split} metrics ===")
    for k, v in rep.items():
        print(f"  {k:24s} {v:.4f}")

    # ---- 1) Fan chart over a window ----
    win = df.iloc[:min(len(df), 480)]                      # ~ up to 10 daylight-days
    ...
    ax.set_title(f"LSTM-Q Probabilistic forecast vs actual — {split} set", color="#E6EDF7")
    ...
    # 2a calibration: nominal vs empirical P(y <= q_level)
    ax = axes[0, 0]
    ...
    # 2c MAE and Pinball loss by hour of the day
    ax = axes[1, 0]
...
def main():
    p = argparse.ArgumentParser(description="Evaluate trained LSTM-Q forecasts")
    p.add_argument("--artifacts", default="artifacts/lstm", help="Path to saved pred parquet files")
