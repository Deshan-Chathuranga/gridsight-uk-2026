...
def _load_neso_frames() -> pd.DataFrame:
    files = sorted(glob.glob(str(BRONZE_LOCAL_DIR / "neso" / "**" / "*.parquet"),
                             recursive=True))
    # Prefer the yearly ARCHIVE files (full history w/ issue time). The rolling
    # "live" snapshot forecasts into the future and would fabricate a huge gap
    # in the canonical grid, so it is excluded from the historical Silver build.
...
def build_silver_neso() -> pd.DataFrame:
    ...
    # 1) timestamp from GMT date + time (period-start, no shift)
    ...
    # 2) anti-leakage: keep latest issue <= slot (rows w/o issue time treated as valid)
