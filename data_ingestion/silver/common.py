...
# separate HF dataset repo for Silver; defaults to bronze repo with bronze->silver
...
TS = "timestamp_utc"  # canonical time column name across all Silver tables

# Business value ranges (used by contracts AND for clamping bad values to NaN).
RANGES = {
...
def apply_missing_policy(
    ...
    """Reindex-aware quality flagging on a df already on the canonical grid.
    ...
    Adds/refreshes a 'data_quality_flag' column.
    """
    ...
    # forward-fill short gaps (only runs whose length <= max_ffill)
...
def write_silver(df: pd.DataFrame, table: str) -> int:
    ...
    if df.empty:
        ...
        return 0
    # clean rebuild: drop any previous partitions so reruns are deterministic
    import shutil
