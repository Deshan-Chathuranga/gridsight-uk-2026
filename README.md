...
flowchart TD
    subgraph Data_Pipeline["1. Data Pipeline (Bronze ➔ Silver ➔ Gold)"]
        A1["NESO / MetOffice / PVLive / OCF APIs"] --> A2["Bronze Layer: Raw Downloads"]
        A2 --> A3["Silver Layer: UTC Aligned & Cleaned"]
        A3 --> A4["Gold Layer: Feature Store"]
    end

    subgraph Model_Engine["2. ML Forecasting & Calibration Engine"]
        A4 --> M1["Model A: TCN-Q + LGBM-Q Stacker"]
        A4 --> M2["Model B: PyTorch LSTM-Q"]
        A4 --> M3["Model C: Pretrained Chronos-Q Benchmark"]
        M1 --> M4["Pinball Loss & 80% PICP Calibration"]
        M2 --> M4
        M3 --> M4
    end

    subgraph Cloud_Storage["3. Cloud Storage & Model Registries"]
        M4 --> S1[("AWS S3 / LocalStack Storage Manager")]
        M4 --> S2[("HuggingFace Hub Datasets & Checkpoints")]
        S1 --> B1
        S2 --> B1
    end

    subgraph Serving_Layer["4. Production Web Serving Layer"]
        B1["FastAPI Application Server (APScheduler)"] --> F1["React Vite TypeScript SPA (Tailwind CSS)"]
    end

    subgraph CICD_Pipeline["5. CI/CD & Automated Deployment"]
        G1["Git Push / PR to main"] --> G2["GitHub Actions CI Workflow"]
        G2 -->|"Pytest Suite"| G3["Unit & Contract Tests Passed"]
        G2 -->|"Docker Build"| G4["Container Validated"]
        G3 --> G5["Railway Cloud Deployment"]
        G4 --> G5
        G5 --> B1
    end
